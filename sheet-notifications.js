(function(){
  ...
  function isLiveOrOpen(r){
    return /\b(open|live|active|apply|applications?\s+open|accepting\s+applications?)\b/i.test(norm(r.status));
  }

  function isUpcomingStatus(r){
    return /\b(upcoming|scheduled|tentative|calendar|to\s*be\s*held|forthcoming)\b/i.test(norm(r.status));
  }

  function isClosed(r){
    return /\b(closed|expired|over|withdrawn|cancelled|declared)\b/i.test(norm(r.status));
  }

  function isSpecialLatestStatus(r){
    return /\b(extended|closing\s*soon)\b/i.test(norm(r.status));
  }
  ...
  function isNew(r){
    ...
    const d=parseDate(r.firstSeen);
    return !!d&&(Date.now()-d.getTime())<=7*86400000;
  }

  function link(r){
    return "job-detail.html?title="+encodeURIComponent(String(r.title||""));
  }
  ...
  function row(r,mode){
    ...
    if(mode==="latest"){
      ...
      const label=/admit\s*card|call\s*letter|hall\s*ticket|e-admit|ecall/.test(m)
        ? "Admit Card"
        : /answer\s*key/.test(m)
          ? "Answer Key"
  ...
  function renderHome(rows){
    ...
    const latest=rows.filter(r=>category(r)==="latest-notifications"&&active(r));
    const upcoming=rows.filter(r=>category(r)==="upcoming-exams"&&active(r));
    const results=rows.filter(r=>category(r)==="results-admit-cards");

    fill(cards[0],latest,"latest");
    fill(cards[1],upcoming,"upcoming");
    fill(cards[2],results,"results");
  }

  Promise.all([
    fetch(DATA_URL+"?v="+Date.now(),{cache:"no-store"})
      .then(r=>{if(!r.ok)throw Error("notifications.json HTTP "+r.status);return r.json()}),
    fetch(DETAILS_URL+"?v="+Date.now(),{cache:"no-store"})
      .then(r=>r.ok?r.json():{})
      .catch(()=>({}))
  ])
  .then(([rows,details])=>{
    if(isHome)renderHome(enrich(Array.isArray(rows)?rows:[],details));
  })
  .catch(err=>{
    console.error("Udhyoga Lakshya Sheets:",err);
    document.querySelectorAll(".sheet-home-loading").forEach(e=>e.textContent="Notifications temporarily unavailable");
  });
})();
