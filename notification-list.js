(function(){
  ...
  function firstSeenTime(v){const s=String(v??"").trim();if(!s)return 0;const d=new Date(s);return Number.isNaN(d.getTime())?0:d.getTime()}
  ...
  const specialLatest=r=>/\b(extended|closing\s*soon)\b/i.test(norm(r.status));
  ...
  const category=r=>{
    if(specialLatest(r))return "latest-notifications";
  ...
  function card(r){const c=document.createElement("article");c.className="notice-item";const d=dateText(r),[label,cls]=statusInfo(r),newBadge=r.isNew?'<span class="status-live">NEW</span>':'';c.innerHTML='<div class="notice-main"><h2><a href="'+esc(detailLink(r))+'">'+esc(r.title)+'</a></h2><div class="meta"><span>'+esc(r.type||"Notification")+'</span><span>'+esc(r.state||"")+'</span>'+(r.category?'<span>'+esc(r.category)+'</span>':'')+(d?'<span>'+esc(d)+'</span>':'')+'<span class="'+cls+'">'+esc(label)+'</span>'+newBadge+'</div></div><a class="details" href="'+esc(detailLink(r))+'">View Details →</a>';return c}
  Promise.all([fetch(DATA_URL+"?v="+Date.now(),{cache:"no-store"}).then(r=>{if(!r.ok)throw Error("HTTP "+r.status);return r.json()}),fetch(DETAILS_URL+"?v="+Date.now(),{cache:"no-store"}).then(r=>r.ok?r.json():{}).catch(()=>({}))]).then(([rows,details])=>{rows=enrich(Array.isArray(rows)?rows:[],details);const f=rows.filter(r=>category(r)===page);f.sort((a,b)=>(firstSeenTime(b.firstSeen)-firstSeenTime(a.firstSeen))||((end(b)?.getTime()||0)-(end(a)?.getTime()||0)));document.getElementById("count").textContent=f.length+" notifications";const list=document.getElementById("list");list.innerHTML="";if(!f.length){list.innerHTML='<div class="empty">No notifications available in this category.</div>';return}f.forEach(r=>list.appendChild(card(r))) }).catch(e=>{console.error(e);document.getElementById("list").innerHTML='<div class="empty">Notifications are temporarily unavailable.</div>'});
})();
